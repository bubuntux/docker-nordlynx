...
|                  `DNS`                   |                | A comma-separated list of IP (v4 or v6) addresses to be set as the interface's DNS servers, or non-IP hostnames to be set as the interface's DNS search domains. *Note*: if not specified, your normal DNS servers will be used leading to a [DNS leak](https://en.wikipedia.org/wiki/DNS_leak). If you do not want to resolve other containers or private services, you can set this to the [NordVPN DNS servers](https://support.nordvpn.com/General-info/1047409702/What-are-your-DNS-server-addresses.htm) and prevent the DNS leak by using `103.86.96.100,103.86.99.100`.
